		...
		<div class="flex items-center gap-5">
			...
			{#if user}
				<div class="flex items-center gap-0.5">
					...
					{#if user.role === 'admin'}
						<a href="/console" class={`text-xs px-3 py-1.5 rounded-lg transition ${isActive('/admin') ? 'text-white bg-white/8' : 'text-white/40 hover:text-white hover:bg-white/5'}`}>Admin</a>
					{/if}
				</div>
			{/if}
		</div>
				...
				{#if open}
					<div class="absolute right-0 top-full mt-2 w-48 bg-[#111] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50">
						...
						<div class="p-1.5 flex flex-col gap-0.5">
							...
							{#if user.role === 'admin'}
								<a href="/console" class="px-3 py-2 text-sm text-white/50 hover:text-white hover:bg-white/5 rounded-lg transition">Admin</a>
							{/if}
	...
	{#if user}
		<nav class="sm:hidden fixed bottom-0 inset-x-0 z-50 bg-black/90 backdrop-blur border-t border-white/8"
			...
			<div class="flex items-center">
				...
				{#if user.role === 'admin'}
					<a href="/console" class={`flex-1 flex flex-col items-center gap-1 py-3 text-xs transition ${isActive('/admin') ? 'text-white' : 'text-white/30'}`}>
						<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8">
